defmodule PrimordialWeb.LiveHelpers do
  ...
  Deprecated should be replaced by core components modal
  ...
  ## Examples
      ...
      <.modal_legacy return_to={Routes.user_index_path(@socket, :index)}>
        <.live_component
    ...
    <div id="modal" class="phx-modal fade-in">
      ...
      >
        <a
          ...
          phx-click="close"
          phx-window-keyup="close_with_key"
          phx-target="#id-card"
        >
          ✖
        </a>
  ...
  def soup_app_drawer(assigns) do
    ...
    <div
      id="drawer"
      class="fixed z-40 p-4 overflow-y-clip border-2
    border-indigo-600 p-5 h-[85vh] md:h-[95vh] lg:h-full w-[85%] md:w-[70%] lg:w-[30%] bg-white
    dark:bg-gray-800 rounded-4xl inset-y-0"
      tabindex="-1"
    >
      <h5
        id="drawer-label"
      ...
      <a
        id="close"
        href="#"
        class="phx-modal-close text-right"
        phx-click="close"
        phx-window-keyup="close_with_key"
        phx-target="#app-drawer"
      >
        ✖
      </a>
      <%= render_slot(@inner_block) %>
  ...
  def clipboard_button(assigns) do
    ...
    <button
      ...
      class="copy-button invisible group-hover:visible top-0 right-0"
      aria-label="copy"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
